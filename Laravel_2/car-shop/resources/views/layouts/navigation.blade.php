            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    @if(auth()->user()->isAdmin())
                        <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.*')" class="me-3">
                            {{ __('Админ панель') }}
                        </x-nav-link>
                    @endif
                    
                    @if(auth()->user()->role === 'customer' || auth()->user()->isAdmin())
                        <x-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.*')" class="me-3">
                            {{ __('Корзина') }}
                        </x-nav-link>
                        
                        <x-nav-link :href="route('profile.orders')" :active="request()->routeIs('profile.orders')" class="me-3">
                            {{ __('Мои заказы') }}
                        </x-nav-link>
                    @endif
                @endauth
